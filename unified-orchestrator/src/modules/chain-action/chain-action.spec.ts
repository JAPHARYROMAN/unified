...
import type { IChainSender, ChainReceipt } from "./chain-sender.types";

// ─── Helpers ──────────────────────────────────────────────────────────────────

type MockAction = {
  id: string; loanId: string; actionKey: null | string; type: "CREATE_LOAN";
  status: "QUEUED" | "SENT" | "PROCESSING" | "MINED" | "FAILED" | "RETRYING";
  payload: Record<string, unknown>; txHash: null | string; nonce: null | number;
  bumpCount: number; blockNumber: null | number; gasUsed: null | bigint;
  revertReason: null | string; error: null | string; attempts: number;
  nextRetryAt: null | Date; sentAt: null | Date; minedAt: null | Date;
  createdAt: Date; updatedAt: Date; loan: { id: string };
};

function mockQueuedAction(overrides?: Partial<MockAction>): MockAction {
  return {
...
function mockSentAction(overrides?: Partial<MockAction>) {
...
describe("C. ChainActionWorker", () => {
  ...
  beforeEach(async () => {
    mockService = {
      ...
      markFailed: jest.fn().mockResolvedValue({ status: "QUEUED" }),
    ...
    const mockMetrics = {
      incSubmitted: jest.fn(), incConfirmed: jest.fn(), incFailed: jest.fn(),
      incDlq: jest.fn(), incNonceConflict: jest.fn(), incRbfBump: jest.fn(),
    };
    const mockSignerNonce = { commit: jest.fn().mockResolvedValue(undefined) };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: TxMetricsService, useValue: mockMetrics },
        { provide: SignerNonceService, useValue: mockSignerNonce },
      ],
    }).compile();
...
describe("D. ChainActionService — markFailed backoff", () => {
  ...
  it("D2: moves to DLQ when max retries (5) reached on transient error", async () => {
    ...
    const svc = new CAS(mockPrisma as any);
    await svc.markFailed("a1", "nonce too low"); // transient — retried until max

    const failCall = mockPrisma.chainAction.update.mock.calls[1][0];
    expect(failCall.data.status).toBe("DLQ");
  });
